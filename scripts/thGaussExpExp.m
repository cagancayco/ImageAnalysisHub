function [thMinimum, thIntersection] = thGaussExpExp(periods)
...
mypdf = @(X, lambda, mu1, sigma, mu2) lambda * normpdf(X, mu1, sigma) + (1 - lambda) * (1/mu2) * exp(X - exp(X)/mu2);
lowerBnd = [0, minPeriods, 0.0000001, exp(medianPeriods)];
upperBnd = [1, maxPeriods, Inf, exp(maxPeriods)];

start = [0.7, medianPeriods, stdPeriods, exp(peakEndModelKernel)];
...
comp2Model = @(X) (1 - lambdaModel) * (1/mu2Model) * exp(X - exp(X)/mu2Model);
diffCompModel = @(X) comp1Model(X) - comp2Model(X);

if diffCompModel(mu1Model) * diffCompModel(log(mu2Model)) < 0
    thIntersection = exp(fzero(diffCompModel, [mu1Model, log(mu2Model)]));
elseif diffCompModel(mu1Model) * diffCompModel(log(mu2Model) + 2*sigma2ModelGaussian) < 0
    thIntersection = exp(fzero(diffCompModel, [mu1Model, log(mu2Model) + 2*sigma2ModelGaussian]));
elseif diffCompModel(mu1Model) * diffCompModel(maxPeriods) < 0
    thIntersection = exp(fzero(diffCompModel, [mu1Model, maxPeriods]));
else
    thIntersection = 0;
end


if log(mu2Model) >= mu1Model
    thMinimum = exp(fminbnd(pdfModel, mu1Model, log(mu2Model)));
else
    thMinimum = exp(fminbnd(pdfModel, mu1Model, maxPeriods));
end
